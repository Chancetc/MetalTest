...
@implementation QGSafeMutableArray


#pragma mark - init

-(instancetype)init{
    INIT(_arr = [NSMutableArray new]);
}
...
- (BOOL)isEqualToArray:(NSArray *)otherArray {
    ...
    if ([otherArray isKindOfClass:QGSafeMutableArray.class]) {
        QGSafeMutableArray *other = (id)otherArray;
        BOOL isEqual;
        [_lock lock];
        [other->_lock lock];
...
- (id)mutableCopyWithZone:(NSZone *)zone {
    LOCK(id copiedArr = [[self.class allocWithZone:zone] initWithArray:_arr]);
    return copiedArr;
}
...
- (BOOL)isEqual:(id)object {
    ...
    if ([object isKindOfClass:[QGSafeMutableArray class]]) {
        QGSafeMutableArray *other = object;
        BOOL isEqual;
        [_lock lock];
        [other->_lock lock];
